...
static struct proc_dir_entry *uart1_proc;

static int ambarella_uart1_proc_read(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int retlen;
	int rcvr;

	rcvr = (ambarella_uart_ports.amba_port[1].fcr & (0x03 << 6)) >> 6;
	retlen = scnprintf(page, count, "UART1, Rx Trigger: %d, ", rcvr);
	switch (rcvr) {
	case 0x00:
		retlen += scnprintf((page + retlen), (count - retlen),
					"One character in the FIFO.\n");
		break;
	case 0x01:
		retlen += scnprintf((page + retlen), (count - retlen),
					"FIFO 1/4 full.\n");
...
static int ambarella_uart1_proc_write(struct file *file,
					...
					unsigned long count, void *data)
{
	...
	if (v < 4) {
		ambarella_uart_ports.amba_port[1].fcr &= ~(0x03 << 6);
		ambarella_uart_ports.amba_port[1].fcr |= (v << 6);

#if 1 // Run-time change?!
		{
			struct uart_port *port = ambarella_uart_ports.amba_port[1].port;

			/* Apply new and reset FIFO. */
			amba_writel(port->membase + UART_FC_OFFSET,
					(ambarella_uart_ports.amba_port[1].fcr |
					 UART_FC_XMITR | UART_FC_RCVRR));
		}
#endif
	} else {
...
{
	uart1_proc = create_proc_entry(uart1_proc_name, S_IRUGO | S_IWUSR,
					get_ambarella_proc_dir());
	if (uart1_proc == NULL) {
		pr_err("%s: %s fail!\n", __func__, uart1_proc_name);
	} else {
		uart1_proc->read_proc = ambarella_uart1_proc_read;
		uart1_proc->write_proc = ambarella_uart1_proc_write;
	}
}
