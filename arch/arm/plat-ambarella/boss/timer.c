...
struct ambarella_cs_timer_info {
	struct notifier_block       system_event;
	struct semaphore        system_event_sem;
};

struct ambarella_cs_timer_info cs_timer_info;
...
void ambarella_cs_timer_change_freq(void)
{
	clocksource_change_rating(&ambarella_cs_timer_clksrc, 0);
	printk(KERN_INFO"%s: before set clksrc.mult = %d", __func__, ambarella_cs_timer_clksrc.mult);
	ambarella_cs_timer_clksrc.mult = clocksource_hz2mult(
	                                     AMBARELLA_TIMER_FREQ, ambarella_cs_timer_clksrc.shift);
	printk(KERN_INFO"%s: after set clksrc.mult = %d", __func__, ambarella_cs_timer_clksrc.mult);
	clocksource_change_rating(&ambarella_cs_timer_clksrc,
	                          AMBARELLA_TIMER_RATING);
}

static int ambarella_cs_timer_system_event(struct notifier_block *nb,
                                           unsigned long val, void *data)
{
	switch (val) {
	case AMBA_EVENT_PRE_CPUFREQ:
		pr_debug("%s: Pre Change\n", __func__);
		down(&cs_timer_info.system_event_sem);
		break;

	case AMBA_EVENT_POST_CPUFREQ:
		pr_debug("%s: Post Change\n", __func__);
		ambarella_cs_timer_change_freq();
		up(&cs_timer_info.system_event_sem);
		break;
...
static inline void ambarella_cs_timer_init(void)
{
	...
	cs_timer_info.system_event.notifier_call =  ambarella_cs_timer_system_event;
	ambarella_register_event_notifier((void*) &cs_timer_info.system_event);
	sema_init(&cs_timer_info.system_event_sem, 1);
#else /* defined(CONFIG_MACH_HYACINTH_0) || defined(CONFIG_MACH_HYACINTH_1) */
	if (machine_is_hyacinth_0()) {
...
static void __init ambarella_timer_init(void)
{
	...
	//boss_set_irq_owner(AMBARELLA_CE_TIMER_IRQ, BOSS_IRQ_OWNER_LINUX, 0);
